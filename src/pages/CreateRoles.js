...
export default function DetailsVehicules() {
  ...
  function AddRoles(){
    ...
    fetch(`${process.env.REACT_APP_API_KEY}/role/create`, {
      method: 'POST',
      headers: {
  ...
  const items = [
    {
      ...
      <Descriptions.Item style={{ display: 'block' }}><input  type='checkbox' id ='Get Invoices'  /> Fetch Data of Invoices</Descriptions.Item>
        ...
        <Descriptions.Item label="Role Name" style={{ display: 'block' }}> <input style={{width: 170, height: 30, marginLeft: 40,borderRadius: 20, border: "1px solid hsl(0, 0%, 80%)"}}  onChange={(event) => setRoleName(event.target.value)}></input></Descriptions.Item>
        <Descriptions.Item label="Description" style={{ display: 'block' }}> <textarea style={{width: 300, height: 80, marginLeft: 40, marginTop: 10, border: "1px solid hsl(0, 0%, 80%)"}} placeholder="......Description" onChange={(event) => setDescription(event.target.value)}></textarea></Descriptions.Item>
        </Descriptions>
       
        </div>
        <button className='my-button ' onClick={AddRoles} style={{fontSize: 15,width: 200,height: 31,padding: "5px 15px",border: "1px solid hsl(0, 0%, 80%)",borderRadius: 10,marginLeft: '35%',display: "flex",justifyContent: "center",alignItems: "center" }}> Create Role</button>
        </from>
