...
export default function EditRentOffer() {
    ...
    const handleSelectChangeVehiculeType = (event) => {
      ...
      useEffect(() => {
        async function fetchDataRentoffer(_id) {
            try {
              ...
              console.log(data)
              const filtredRentoffer = data.filter((item) => item._id == _id);
              console.log(filtredRentoffer)
              setPrix(filtredRentoffer[0]?.prix)
              setMaxDistance(filtredRentoffer[0]?.maxDistance)
              setDescription(filtredRentoffer[0]?.description.join('\n'))
              setPeriod(filtredRentoffer[0]?.period)
              setVehiculeType(filtredRentoffer[0]?.vehiculeType?.vehicule?._id)
              setVehicleDescription(filtredRentoffer[0]?.vehiculeType?.vehiculeDescription.join('\n'))
              set_id(_id)
            } catch (error) {
              console.error(error);
            }
          }
          fetchDataRentoffer(_id);
    ...
    function handleSubmitRentoffer(event) {
      ...
      fetch(`${process.env.REACT_APP_API_KEY}/rentoffer/modify`, {
        method: 'POST',
        headers: {
          'Content-type': 'application/json',
          'authorization':  JSON.parse(localStorage.getItem('login'))?.token
    ...
    const steps = [
        {
          ...
          <textarea type="text" className="input-vehiculedetails"name='description' value={description} onChange={(event) => setDescription(event.target.value)} required/><br />
          <label className='p-vehiculedetails'></label><br /><br />
          ...
          <button type="submit" className='my-button ' style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} onClick={handleSubmitRentoffer}>Edit</button>
          </div>
