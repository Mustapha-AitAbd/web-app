...
export default function EditPackage() {
    ...
    useEffect(() => {
        async function fetchDataPackage(_id) {
            try {
              const response = await fetch(`${process.env.REACT_APP_API_KEY}/package`,{
                method:'POST',
              ...
              const filteredPackage = data.filter((item) => item._id == _id);
              setPackageName(filteredPackage[0]?.packageName)
              setCreditsOffer(filteredPackage[0]?.creditsOffer)
              setPrice(filteredPackage[0]?.price)
              set_id(_id)
            } catch (error) {
              console.error(error);
            }
          }
          fetchDataPackage(_id);
    ...
    function handleSubmitPackage(event) {
      ...
      fetch(`${process.env.REACT_APP_API_KEY}/package/modify`, {
        method: 'POST',
        headers: {
          'Content-type': 'application/json',
          'authorization':  JSON.parse(localStorage.getItem('login'))?.token
